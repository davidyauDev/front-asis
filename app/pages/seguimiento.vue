...
<template>
        ...
        <template #right>
          <div class="flex items-center gap-2">
            ...
            <UTooltip text="Técnicos sin marcación">
              ...
              >
                <div class="relative">
                  <UIcon name="i-heroicons-bell" class="w-5 h-5 text-gray-500 dark:text-gray-400 group-hover:text-gray-700 dark:group-hover:text-gray-300 transition-colors" />
    ...
    <template #body>
         <div class="p-2">
            <div class="bg-white dark:bg-gray-950 rounded-lg border border-gray-200/60 dark:border-gray-800/60 p-5 mb-6 shadow-sm">
                <div class="flex flex-wrap gap-2 items-center">
                    <div class="relative flex-1 min-w-[300px]">
                        <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-gray-400 dark:text-gray-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <div class="relative">
                        <select v-model="filtroEstado" class="appearance-none border border-gray-200/80 dark:border-gray-800/80 rounded-md pl-3 pr-8 py-2.5 bg-white dark:bg-gray-950 focus:border-gray-400 dark:focus:border-gray-600 focus:ring-0 transition-all duration-200 text-sm text-gray-700 dark:text-gray-300 cursor-pointer hover:border-gray-300 dark:hover:border-gray-700">
                    ...
                    <button class="bg-gray-900 dark:bg-gray-100 text-white dark:text-gray-900 px-4 py-2.5 rounded-md hover:bg-gray-800 dark:hover:bg-gray-200 transition-all duration-200 flex items-center gap-2 text-sm font-medium shadow-sm hover:shadow" @click="generarReporte">
                ...
                >
                    ...
                    <div class="flex items-center justify-between">
                        ...
                        <div v-if="card.titulo === 'Sin Marcación' && card.valor > 0">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-7 w-7 text-red-500/70" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div class="mb-6">
                <div class="flex gap-1 border-b border-gray-200/60 dark:border-gray-800/60">
                    <button
                        ...
                        class="px-5 py-3 text-sm font-medium transition-all duration-200 relative"
                        :class="{
        ...
        <section>
            <div v-if="isLoading" class="flex flex-col items-center justify-center py-20">
                <svg class="animate-spin h-10 w-10 text-gray-950 dark:text-gray-50 mb-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
            ...
            <div v-else-if="error" class="bg-red-50 dark:bg-red-950/20 border border-red-200/60 dark:border-red-900/60 rounded-lg p-8 text-center shadow-sm">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-12 w-12 text-red-500/70 dark:text-red-400/70 mx-auto mb-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div class="mb-5 text-sm bg-white dark:bg-gray-950 rounded-lg p-5 border border-gray-200/60 dark:border-gray-800/60 shadow-sm">
                <div class="flex items-center justify-between mb-4">
                    <span class="font-medium text-gray-600 dark:text-gray-400">
                        Mostrando <span class="font-semibold text-gray-950 dark:text-gray-50">{{ datosAgrupadosFiltrados.length }}</span> técnico(s) con rutas
                        <span v-if="search || filtroEstado !== 'todos'" class="text-gray-600 dark:text-gray-400 bg-gray-100 dark:bg-gray-800/80 px-2 py-0.5 rounded text-xs ml-1">(filtrado)</span>
                    </span>
                <button 
                ...
                <div class="flex items-center gap-4">
                    <div class="flex items-center gap-2">
                        ...
                        <span class="text-xs text-gray-600 dark:text-gray-400">
                            <span class="font-semibold text-emerald-700 dark:text-emerald-400">{{ tecnicosConRutasConMarcacion }}</span> marcaron
                        </span>
                    </div>
                    <div class="flex items-center gap-2">
                        <div class="w-2 h-2 rounded-full bg-red-500/80"></div>
                        <span class="text-xs text-gray-600 dark:text-gray-400">
                            <span class="font-semibold text-red-700 dark:text-red-400">{{ tecnicosConRutasSinMarcacion }}</span> no marcaron
                        </span>
                    </div>
                </div>
            </div>
            ...
            <div class="space-y-4 max-h-[calc(100vh-400px)] overflow-y-auto pr-2">
                <div v-if="datosAgrupadosFiltrados.length === 0" class="text-center py-16 bg-gray-50/50 dark:bg-gray-900/30 rounded-lg border border-gray-200/60 dark:border-gray-800/60">
                    ...
                    <p class="text-gray-700 dark:text-gray-300 font-medium text-sm mb-1">No se encontraron técnicos</p>
                    <p class="text-xs text-gray-500 dark:text-gray-500">Intenta ajustar los filtros de búsqueda</p>
                </div>
                ...
                >
                    <div 
                        class="p-5 cursor-pointer transition-all duration-200"
                        :class="{
                    ...
                    >
                        <div class="flex items-center justify-between">
                            <div class="flex items-center gap-3">
                                <div class="w-11 h-11 bg-gray-900 dark:bg-gray-100 rounded-md flex items-center justify-center text-white dark:text-gray-900 font-medium text-sm shrink-0 shadow-sm">
                                    {{ tecnicoData.rutas[0]?.firstname?.charAt(0) || '' }}{{ tecnicoData.rutas[0]?.lastname?.charAt(0) || '' }}
                                </div>
                                
                                <div class="min-w-0">
                                    <h3 class="text-sm font-medium text-gray-950 dark:text-gray-50 truncate">
                                        {{ tecnicoData.rutas[0]?.firstname || '' }} {{ tecnicoData.rutas[0]?.lastname || '' }}
                                    </h3>
                                    <p class="text-xs mt-0.5">
                                        <span v-if="dni && dni !== 'null' && dni !== 'undefined'" class="text-gray-500 dark:text-gray-400">DNI: {{ dni }}</span>
                                        <span v-else class="bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400 px-2 py-0.5 rounded-md font-medium">Sin técnico asignado</span>
                                    </p>
                                </div>
                            </div>
                            ...
                            <div class="flex items-center gap-2 shrink-0">
                                ...
                                <span class="px-2.5 py-1 bg-gray-100/80 dark:bg-gray-800/80 text-gray-700 dark:text-gray-300 text-xs font-medium rounded-md" :title="`${tecnicoData.rutas.length} ruta(s)`">
                                    {{ tecnicoData.rutas.length }} rutas
                                </span>

                                <!-- Badge que muestra si existe daily_record (click para ver modal) -->
                                <button v-if="getDailyRecord(tecnicoData)" @click.stop="viewDailyRecord(tecnicoData)" class="ml-2 px-2 py-1 bg-indigo-50 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-300 text-xs font-medium rounded-md">
                                    Registro
                                </button>
                                ...
                                <div v-if="getDailyRecord(tecnicoData) && (esObjetoSinMarcacion(tecnicoData.iclock_transactions) || (Array.isArray(tecnicoData.iclock_transactions) && tecnicoData.iclock_transactions.length === 0))" class="flex items-center gap-2">
                                    <button @click.stop="toggleDailyRecord(tecnicoData)" class="px-2 py-1 bg-gray-100 dark:bg-gray-800 rounded-md text-xs text-gray-700 dark:text-gray-300">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 inline-block mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M12 20h.01" />
                                        </svg>
                                        Registro
                                    </button>
                                    <div v-if="showDailyRecord[getKeyForTecnico(tecnicoData)]" class="mt-2 p-3 bg-gray-50 dark:bg-gray-900 rounded text-xs w-full border border-gray-200/60 dark:border-gray-800/60">
                                        <pre class="whitespace-pre-wrap">{{ formatDailyRecord(getDailyRecord(tecnicoData)) }}</pre>
                                    </div>
                                </div>
                    ...
                    <div v-show="tecnicosExpandidos[dni]" class="p-6 bg-gray-50/50 dark:bg-gray-900/20 border-t border-gray-200/60 dark:border-gray-800/60">
                        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                            <div>
                                <div class="flex items-center gap-2.5 mb-5 pb-3 border-b border-gray-200/60 dark:border-gray-800/60">
                                    <div class="w-9 h-9 bg-gray-100 dark:bg-gray-800/80 rounded-md flex items-center justify-center">
                                    ...
                                    <h4 class="text-sm font-medium text-gray-950 dark:text-gray-50">Rutas Asignadas</h4>
                                </div>
                                <div class="space-y-3 max-h-96 overflow-y-auto pr-2">
                                    ...
                                    >
                                        <div class="flex items-start justify-between mb-2.5">
                                            <div class="flex items-center gap-2">
                                                <span class="text-xs font-medium bg-gray-100 dark:bg-gray-800/80 px-2.5 py-1 rounded-md text-gray-700 dark:text-gray-300">{{ ruta.number }}</span>
                                                <span
                                            ...
                                            <span class="text-xs text-gray-500 dark:text-gray-400">{{ ruta.fecha_programada?.split(' ')[1] }}</span>
                                        </div>
                                        <p class="text-sm font-medium text-gray-950 dark:text-gray-50 mb-2.5">
                                            {{ ruta.agencia }}
                                        </p>
                                        <div class="space-y-1.5 text-xs text-gray-600 dark:text-gray-400">
                                            <p>
                                                <span class="font-medium text-gray-700 dark:text-gray-300">Equipo:</span> {{ ruta.equipo }}
                                            </p>
                                            <p>
                                                <span class="font-medium text-gray-700 dark:text-gray-300">Servicio:</span> {{ ruta.topic }}
                                            </p>
                                            <p>
                                                <span class="font-medium text-gray-700 dark:text-gray-300">Cliente:</span> {{ ruta.cliente }}
                                            </p>
                                        </div>
                                        <p v-if="ruta.subject !== 'S/N'" class="text-xs text-gray-500 dark:text-gray-400 mt-2.5 pt-2.5 border-t border-gray-100 dark:border-gray-800/60">{{ ruta.subject }}</p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div>
                                <div class="flex items-center gap-2.5 mb-5 pb-3 border-b border-gray-200/60 dark:border-gray-800/60">
                                    ...
                                    <h4 class="text-sm font-medium text-gray-950 dark:text-gray-50">Marcaciones de Asistencia</h4>
                                </div>
                                <div
                                    v-if="esObjetoSinMarcacion(tecnicoData.iclock_transactions)"
                                    class="bg-red-50 dark:bg-red-950/20 border border-red-200/60 dark:border-red-900/60 rounded-lg p-5 text-center shadow-sm"
                                >
                                    <div class="inline-flex items-center justify-center w-11 h-11 bg-red-100 dark:bg-red-900/30 rounded-full mb-3">
                                    ...
                                    <p class="text-sm font-medium text-red-700 dark:text-red-300 mb-1">Sin registro de marcación</p>
                                    <p class="text-xs text-red-600/80 dark:text-red-400/80">El técnico no ha registrado asistencia hoy</p>
                                </div>
                                        ...
                                        <div class="flex items-start justify-between gap-3">
                                            <div class="flex-1 min-w-0">
                                                <div class="flex items-center justify-between mb-2.5">
                                                    <div>
                                                        <p class="text-sm font-medium text-gray-950 dark:text-gray-50">
                                                            {{ new Date(marcacion.punch_time).toLocaleTimeString('es-PE', { hour: '2-digit', minute: '2-digit' }) }}
                                                        </p>
                                                        <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">{{ marcacion.terminal_sn }}</p>
                                                    </div>
                                                </div>
            ...
            <div v-show="tabActivo === 'sin-rutas'" class="animate-fadeIn">
                <div class="mb-5 text-sm bg-white dark:bg-gray-950 rounded-lg p-5 border border-gray-200/60 dark:border-gray-800/60 shadow-sm">
                    <div class="flex items-center justify-between mb-4">
                        <span class="font-medium text-gray-600 dark:text-gray-400">
                            Mostrando <span class="font-semibold text-gray-950 dark:text-gray-50">{{ tecnicosSinRutasFiltrados.length }}</span> técnico(s) sin rutas
                            <span v-if="search" class="text-gray-600 dark:text-gray-400 bg-gray-100 dark:bg-gray-800/80 px-2 py-0.5 rounded text-xs ml-1">(filtrado)</span>
                        </span>
                    </div>
                    <div class="flex items-center gap-4">
                        <div class="flex items-center gap-2">
                            <div class="w-2 h-2 rounded-full bg-emerald-500/80"></div>
                            <span class="text-xs text-gray-600 dark:text-gray-400">
                                <span class="font-semibold text-emerald-700 dark:text-emerald-400">{{ tecnicosSinRutasConMarcacion }}</span> marcaron
                            </span>
                        </div>
                        <div class="flex items-center gap-2">
                            <div class="w-2 h-2 rounded-full bg-red-500/80"></div>
                            <span class="text-xs text-gray-600 dark:text-gray-400">
                                <span class="font-semibold text-red-700 dark:text-red-400">{{ tecnicosSinRutasSinMarcacion }}</span> no marcaron
                            </span>
                        </div>
                    </div>
                </div>
                ...
                <div class="space-y-4 max-h-[calc(100vh-400px)] overflow-y-auto pr-2">
                    <div v-if="tecnicosSinRutasFiltrados.length === 0" class="text-center py-16 bg-gray-50/50 dark:bg-gray-900/30 rounded-lg border border-gray-200/60 dark:border-gray-800/60">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-12 w-12 mx-auto text-gray-400 dark:text-gray-600 mb-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                        </svg>
                        <p class="text-gray-700 dark:text-gray-300 font-medium text-sm mb-1">Todos los técnicos tienen rutas asignadas</p>
                        <p class="text-xs text-gray-500 dark:text-gray-500">No hay técnicos sin rutas para la fecha seleccionada</p>
                    </div>

                    <div
                        v-for="tecnico in tecnicosSinRutasFiltrados"
                        :key="tecnico.dni"
                            ...
                            <div class="flex items-center justify-between">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div class="min-w-0">
                                        ...
                                        <p class="text-xs mt-0.5">
                                            <span v-if="tecnico.dni && tecnico.dni !== 'null'" class="text-gray-500 dark:text-gray-400">DNI: {{ tecnico.dni }}</span>
                                            <span v-else class="bg-gray-100 dark:bg-gray-800/80 text-gray-600 dark:text-gray-400 px-2 py-0.5 rounded-md font-medium">Sin DNI</span>
                                        </p>
                                    </div>
                                </div>
                                
                                <div class="flex items-center gap-2 shrink-0">
                                    <span class="px-2.5 py-1 bg-amber-100/80 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400 text-xs font-medium rounded-md" title="Sin rutas asignadas">
                                        Sin rutas
                                    </span>
                                    ...
                                    <button v-if="getDailyRecord(tecnico)" @click.stop="viewDailyRecord(tecnico)" class="ml-2 px-2 py-1 bg-indigo-50 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-300 text-xs font-medium rounded-md">Registro</button>
                                ...
                                <div v-if="showDailyRecord[getKeyForTecnico(tecnico)]" class="mt-2 p-3 bg-gray-50 dark:bg-gray-900 rounded text-xs w-full border border-gray-200/60 dark:border-gray-800/60">
                                    <pre class="whitespace-pre-wrap">{{ formatDailyRecord(getDailyRecord(tecnico)) }}</pre>
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        <ValidationModal v-model:show="showValidationModal" :tecnico="validationTarget" @submit="onValidationSubmit" />
        ...
        <div v-if="showDailyModal" class="fixed inset-0 z-50 flex items-center justify-center">
            ...
            <div class="relative bg-white dark:bg-gray-950 rounded-lg shadow-lg w-full max-w-2xl mx-4 p-5" role="dialog" aria-modal="true">
                <header class="flex items-center justify-between mb-4">
                    <h3 class="text-lg font-medium text-gray-900 dark:text-gray-50">Detalle de Registro</h3>
                    <button @click="showDailyModal = false" class="text-gray-500 hover:text-gray-700 dark:hover:text-gray-300">✕</button>
                </header>
                <pre class="text-xs overflow-auto max-h-[60vh] p-2 bg-gray-50 dark:bg-gray-900 rounded">{{ formatDailyRecord(dailyModalContent) }}</pre>
            </div>
        </div>
        </template>
    </UDashboardPanel>
