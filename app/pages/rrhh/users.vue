...
const isUsersRoot = shallowRef(true)
const isBioTimeRoute = shallowRef(false)
const usersTab = shallowRef<'users' | 'biotime'>('users')
const visibleUsersTabItems = shallowRef([
  { label: 'Usuarios', value: 'users' },
  { label: 'Empleados BioTime', value: 'biotime' },
])
...
<template>
  <UDashboardPanel id="users">
    <template #header>
      ...
      <div v-if="false" class="mx-4 mt-2">
        <div class="rounded-t-lg border border-gray-200 border-b-0 bg-white dark:border-gray-800 dark:bg-gray-950 overflow-hidden">
          <AppTabs
            v-model="usersTab"
            aria-label="Tabs de usuarios"
            :items="visibleUsersTabItems"
          />
        </div>
        <div v-if="false" class="rounded-t-lg border border-gray-200 dark:border-gray-800 border-b-0 bg-white dark:bg-gray-950 overflow-hidden">
